...
void ImageItem::Draw()
{
    ...
    if (GetSubMenu())
    {
        ...
        if (m_HasIcon)
            cTargetRect = cSourceRect.Bounds() + Point( m->GetBounds().right - SUB_MENU_ARROW_W - m_Bitmap->GetBounds().Width() + 9, m->GetBounds().top + ( m->GetBounds().Height() / 7) - 4.0f);



        else
            cTargetRect = cSourceRect.Bounds() + Point( m->GetBounds().right - SUB_MENU_ARROW_W, m->GetBounds().top + ( m->GetBounds().Height() / 7) - 4.0f);
